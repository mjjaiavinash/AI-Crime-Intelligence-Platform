    ...
    <aside className={`flex flex-col h-screen bg-surface-50 border-r border-slate-700/40
      ...
      <div className="border-t border-slate-700/40 p-2.5">
        <div className={`flex items-center gap-2.5 ${collapsed ? 'justify-center' : 'px-1'}`}>
          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-xs font-bold text-white flex-shrink-0 shadow-md">
            {user?.username?.[0]?.toUpperCase() ?? 'A'}
          </div>
          {!collapsed && (
            <>
              <div className="flex-1 overflow-hidden min-w-0">
                <p className="text-sm font-semibold text-white truncate leading-tight">{user?.username ?? 'Analyst'}</p>
                <p className="text-[10px] text-emerald-400 truncate leading-tight">Crime Analyst</p>
              </div>
