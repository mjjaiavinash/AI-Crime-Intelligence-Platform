...
const COLUMNS = [
  ...
  { key: 'age_at_incident', label: 'Age',      render: (r) => <span className="text-slate-300">{r.age_at_incident ?? '—'}</span> },
  { key: 'fir_id',          label: 'FIR ID',   render: (r) => <span className="font-mono text-xs text-primary-300">#{r.fir_id}</span> },
...
export default function InvestigatorVictims() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail ?? 'Failed to add victim')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setOpen(true)} className="btn-primary text-sm">+ Add Victim</button>
          </div>
        }
      />
      ...
      <div className="card">
        <div className="px-5 py-4 border-b border-slate-700/50 flex items-center gap-4">
          ...
          <input
            ...
            placeholder="Search name, injury, gender…"
            value={search}
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Add Victim">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving} className="btn-primary flex-1">
              {saving ? <Spinner size="sm" /> : 'Add Victim'}
            </button>
          </div>
        </form>
      </Modal>
